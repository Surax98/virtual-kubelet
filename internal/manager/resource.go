...
func (rm *ResourceManager) GetSecret(name, namespace string) (*v1.Secret, error) {
	...
	beforesecret, err := rm.secretLister.Secrets(namespace).List(labels.Everything())
	fmt.Println(beforesecret, err)
	configmaps, err := rm.configMapLister.ConfigMaps(namespace).List(labels.Everything())
	fmt.Println(configmaps, err)
